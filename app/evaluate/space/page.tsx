...
export default function SpaceDetailPage() {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <PageIntro
        step="STEP 2 / 3"
        title="把现场会影响居住体验的细节补进去。"
        description="这一页主要关注采光、临街、潮湿、通风和动线结构。哪怕你只知道大概情况，也能先帮系统筛出明显风险。"
        icon={Sun}
        chips={["约 1 分钟", "先看风险源", "支持不完全信息"]}
      />

      <FormSection
        title="采光与日照"
        description="优先判断卧室和主要活动区域会不会过热、过暗，尤其是下午西晒和窗外遮挡。"
        icon={Sun}
      >
        <ToggleCard
      ...
      <FormSection
        title="噪声与外部干扰"
        description="这一部分主要识别睡眠和专注工作的干扰源，尤其是临街、靠电梯和户型边角位。"
        icon={Wind}
      >
      ...
      <FormSection
        title="户型结构"
        description="厨房、卫生间和阳台位置决定了气味、潮湿和生活动线，往往比装修风格更影响日常感受。"
        icon={Building2}
      >
        <ToggleCard
