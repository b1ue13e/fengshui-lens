...
import { EngineResult, type Dimension } from "@/types";
...
function analyzeUncertainty(report: EngineResult): ReportAlert[] {
  ...
  if (topGap < 5 && dims[0] && dims[1]) {
    alerts.push({
      type: "info",
      title: "优势项并不绝对",
      message: `${dimensionLabel[dims[0].dimension]}和${dimensionLabel[dims[1].dimension]}的得分接近，说明这套房没有特别突出的单一优势。`,
      suggestion: "现场再重点确认这两个维度里，哪个对你更重要。",
    });
  }
  ...
  if (report.overallScore >= 70 && report.overallScore <= 80) {
    alerts.push({
      type: "info",
      title: "当前是临界分段",
      message: `总分 ${report.overallScore} 落在“可以继续看”和“需要带着问题看”的边界区间。`,
  ...
  if (report.overallScore >= 50 && report.overallScore <= 60) {
    alerts.push({
      type: "warning",
      title: "接近放弃阈值",
      message: `总分 ${report.overallScore} 已经靠近“不建议继续投入时间”的分界线。`,
      suggestion: "除非房东愿意让价或问题能低成本修正，否则继续谈的性价比不高。",
    });
  }
  ...
  if (photoDependentRisks.length > 0) {
    alerts.push({
      ...
      suggestion: "看房时记得重点查看墙角、窗边和卫浴附近。",
    });
  }
    ...
    return (
      <section className="paper-panel rounded-[1.8rem] p-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-base font-medium text-foreground">当前报告置信度较高</h3>
            <p className="mt-2 text-sm leading-7 text-muted-foreground">
              基于你提供的信息，这份预筛报告没有明显的临界或冲突信号，适合作为继续判断的第一参考。
            </p>
          </div>
        </div>
      </section>
